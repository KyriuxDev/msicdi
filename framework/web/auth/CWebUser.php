...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	public function isAdministrador(){
			$query = "SELECT Rol FROM cdi_usuarios WHERE Matricula='".Yii::app()->user->name."'";
			$res = Yii::app()->db->createCommand($query)->queryAll();
			if(count($res)>0)
				return ($res[0]['Rol']=='admin')?true:false;	
			else
				return false;	
	}

	public function getShortName(){
		$query = "SELECT CONCAT(Nombres,' ',ApPaterno) nom FROM cdi_cat_personal WHERE Matricula = '".Yii::app()->user->name."'";
		$res = Yii::app()->db->createCommand($query)->queryAll();
		return $res[0]['nom'];
	}

	public function getFullName(){
		$query = "SELECT CONCAT(Nombres,' ',ApPaterno,' ',ApMaterno) nom FROM cdi_cat_personal WHERE Matricula = '".Yii::app()->user->name."'";
		$res = Yii::app()->db->createCommand($query)->queryAll();
		return $res[0]['nom'];
	}

	public function getShortNameById($matr){
		$query = "SELECT CONCAT(Nombres,' ',ApPaterno) nom FROM cdi_cat_personal WHERE Matricula = '{$matr}'";
		$res = Yii::app()->db->createCommand($query)->queryAll();
		if( sizeof($res)>0 )
			return $res[0]['nom'];
		else
	...
	public function isCoAdmin(){
			...
			$res = Yii::app()->db->createCommand($query)->queryAll();
			if(count($res)>0)
				return ($res[0]['Rol']=='coadmin')?true:false;	
			else
				return false;	
	}

	public $nombreCorto  = "GUEST";
	/**
	 * @var boolean whether to know if its a Co-Admin login. Defaults to false.
	 */
	public $isCoAdmin=false;
	...
	public function init()
	{
		...
		if( $this->name != 'Guest'){
			$query = "SELECT concat(ApPaterno,' ',SUBSTRING_INDEX(Nombres, ' ', 1) ) nom from cdi_cat_personal p  where p.Matricula='{$this->name}'";
			$res = Yii::app()->db->createCommand($query)->queryAll();
			$this->nombreCorto = $res[0]['nom'];
		}
	}
