...
require_once("include/reportesaio_variables.php");
...
$page_layouts["reportesaio_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["num"]));
	
	
	//	num - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("num", $data, $keylink);
			$row["num_value"] = $value;
			$format = $pSet->getViewFormat("num");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("num")))
	...
	//	serie - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("serie", $data, $keylink);
			$row["serie_value"] = $value;
			$format = $pSet->getViewFormat("serie");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("serie")))
				$class = ' rnr-field-number';
			$row["serie_class"] = $class;
	//	unidad - 
			$viewContainer->recId = $recordsCounter;
	...
	//	falla - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("falla", $data, $keylink);
			$row["falla_value"] = $value;
			$format = $pSet->getViewFormat("falla");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("falla")))
				$class = ' rnr-field-number';
			$row["falla_class"] = $class;
	//	fecha - Short Date
			$viewContainer->recId = $recordsCounter;
	...
	//	ip - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ip", $data, $keylink);
			$row["ip_value"] = $value;
			$format = $pSet->getViewFormat("ip");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("ip")))
				$class = ' rnr-field-number';
			$row["ip_class"] = $class;
	//	cuenta - 
			$viewContainer->recId = $recordsCounter;
	...
	//	evidencia - File-based Image
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("evidencia", $data, $keylink);
			$row["evidencia_value"] = $value;
			$format = $pSet->getViewFormat("evidencia");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("evidencia")))
				$class = ' rnr-field-number';
			$row["evidencia_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("reportesaio", "detailspreview"));
