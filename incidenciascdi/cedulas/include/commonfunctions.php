...
function checkTableName($shortTName, $type=false)
{
	...
	if ("cat_adscripcion" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("cat_criterios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("cat_personal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("datos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("respuestas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetUserPermissions($table="")
{
	...
	if(!$table)
		$table = $strTableName;
	$permissions = "";
	if($table=="cat_adscripcion")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="cat_criterios")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="cat_personal")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="datos")
