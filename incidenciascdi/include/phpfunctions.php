...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="win10" && $field=="entidad") 
	{
		return OAXACA;
	}
				if($table=="win10" && $field=="delegacion") 
	{
		return OAXACA;
	}
				if($table=="win10" && $field=="fecha") 
	{
		return now();
	}
				if($table=="reportesaio" && $field=="fecha") 
	{
		return now();
	}
	return "";
...
function GetUploadFolderExpression($field, $file, $table="")
{
	...
	{
		$year=GetCurrentYear();
$folder = "files/incidenciascdi/".$year;;
		return $folder;
	}
	if($table=="reportesaio" && $field=="evidencia") 
	{
		$year=GetCurrentYear();
$folder = "files/incidenciascdi/".$year;;
		return $folder;
