<?php
	...
	Class SoporteController extends Controller{
			...
			if(Yii::app()->user->isGuest){
				...
				if( Yii::app()->user->isCoAdmin() ){
					...
					if( Yii::app()->user->isAdministrador() ){
					    ...
					    $isAdmin = Yii::app()->user->isAdministrador();
					    foreach ($models as $model) {
							if ($model['nombreReportador'] == '' && $model['Matricula'] != '') {
								$model['nombreReportador'] = $this->getNombreByMatricula($model['Matricula']);
							}
						}
		 ...
		 * @since 1.0.1 Se introdujo este elemento
		 * Determinar el reportador con prioridad:
			// 1. Nombre LDAP (si tiene correo institucional válido)
			// 2. Matrícula (si la proporcionó)
			// 3. Nombre manual (si marcó "no tengo matrícula")
		 */
		public function actionReporteEnviado(){
			...
			$ldapNombre   = isset($_POST['ldap_nombre']) ? trim($_POST['ldap_nombre']) : '';
			$nmatr        = isset($_POST['nmatr'])        ? trim($_POST['nmatr'])        : '';
			$conMatricula = true;
			$gReportador  = '';

			if ($ldapNombre !== '') {
				$gReportador  = strtoupper($ldapNombre);
				$conMatricula = false;
			} elseif (isset($_POST['NoTengoMatricula'])) {
				$gReportador  = trim($_POST['nnom']);
				$conMatricula = false;
				$nmatr        = ''; // sin matrícula
			} else {
				$gReportador  = $nmatr;
			...
			$nombreOverride = ($ldapNombre !== '') ? strtoupper($ldapNombre) : '';

			if ($this->insertaRegistro($idticket, $nmatr, $dFalla, $nSerie, $ipOrigen, $conMatricula, $nombreOverride))
		...
		public function actionReporteEnviadoManual(){
			if( isset($_POST['nserie']) ){
				...
				$dFalla 		= $_POST['falla'];
				$ipOrigen 		= $_POST['ipOrigen'];
				$correo      	= isset($_POST['ncorr'])     ? trim($_POST['ncorr'])     : '';
				$telefono 		= $_POST['ntel'];
				...
				if ($ldapNombre !== '') {
					// CASO 1: Con matrícula + LDAP
					$gReportador    = strtoupper($ldapNombre);
					$conMatricula   = true;
					$nombreOverride = strtoupper($ldapNombre);

				} elseif (!isset($_POST['NoTengoMatricula']) && $nmatr !== '') {
					// CASO 2: Con matrícula + sin LDAP (busca nombre en BD)
					$gReportador    = $nmatr;
					$conMatricula   = true;
					$nombreOverride = '';

				} else {
					// CASO 3: Sin matrícula + sin LDAP (nombre escrito manualmente)
					$gReportador    = trim($_POST['nnom']);
					$conMatricula   = false;
					$nmatr          = '';
					$nombreOverride = trim($_POST['nnom']);
				}
				$directorioArchivos = "/upload";
				//$imginfo = getimagesize($_FILES['primerArchivo']['tmp_name']); 
				...
				//$imginfo = getimagesize($_FILES['segundoArchivo']['tmp_name']); 
				//if (($imginfo[0] <= 700) && ($imginfo[1] <= 438)) {
				...
				//$imginfo = getimagesize($_FILES['tercerArchivo']['tmp_name']); 
				//if (($imginfo[0] <= 700) && ($imginfo[1] <= 438)) {
				...
				if ($this->insertaRegistroManual($idticket, $nmatr, $dFalla, $nSerie, $ipOrigen, $conMatricula, $correo, $telefono, $nUsuario, $ncontrasena, $nIpMaquina, $depto, $nombreOverride)){
		...
		public function actionRastreo(){
			if(isset($_GET['codigo']))
				if ($this->isCodigoValido($_GET['codigo'])){
					...
					if($isAdmin){
						$query = "SELECT p.Matricula,CONCAT(p.Matricula,' - ',p.Nombres,' ',p.ApPaterno,' ',ApMaterno) nom 
								 FROM cdi_usuarios u,cdi_cat_personal p 
								 WHERE (Rol='Admin' or Rol='CoAdmin') and p.Matricula=u.Matricula
								 ORDER BY p.Nombres, p.ApPaterno, p.ApMaterno";
						...
						if( $isCoAdmin){
							$query = "SELECT p.Matricula,CONCAT(p.Matricula,' - ',p.Nombres,' ',p.ApPaterno,' ',ApMaterno) nom 
									  FROM cdi_usuarios u,cdi_cat_personal p 
									  WHERE (Rol='Admin') and p.Matricula=u.Matricula
									  ORDER BY p.Nombres, p.ApPaterno, p.ApMaterno";
							$resD 			= Yii::app()->db->createCommand($query)->queryAll();
		 ...
		 * @since 1.0.1 Se introdujo este elemento
		 */
		private function insertaRegistro($id, $matr, $reporte, $serie, $ipO, $conMatricula, $nombreOverride = ''){
			if ($conMatricula) {
				// Si viene nombre LDAP, usarlo; si no, buscarlo en BD
				if ($nombreOverride !== '') {
					$nom = $nombreOverride;
				} else {
		 ...
		 * @since 1.0.1 Se introdujo este elemento
		 */
		private function insertaRegistroManual($id, $matr, $reporte, $serie, $ipO, $conMatricula, $correo, $telefono, $nUsuario, $ncontrasena, $nIpMaquina, $depto, $nombreOverride = ''){
			...
			if ($conMatricula) {
				if ($nombreOverride !== '') {
					$nom           = $nombreOverride;
					$nomReportador = $nombreOverride;
				} else {
					$query = "SELECT concat(Nombres,' ', ApPaterno,' ',ApMaterno) nom from cdi_cat_personal p where p.Matricula='{$matr}'";
					$res   = Yii::app()->db->createCommand($query)->queryAll();
					// Verifica que exista antes de acceder
					if (empty($res)) {
						Yii::log("Matrícula no encontrada en BD: {$matr}", 'warning');
						$this->render('fallido');
						return false;
					}
					$nom           = $res[0]['nom'];
				...
				$li    = '<li class="smaller">'.date("Y-m-d H:i:s").' | Reporte enviado por: '.$nom.'</li>';
				$query  = "INSERT INTO cdi_reportes_manual (NRastreo, nombreReportador, Matricula, eMail, nSerie, descripcionFalla, fechaReporte, status, ultimoCambio, tiempoTranscurrido, soporte, historial, ipOrigen, Telefono, usuario, contrasena, ipEquipo, departamento) VALUES ";
				$query .= "('{$id}', '{$nomReportador}', '{$matr}', '{$correo}', '{$serie}', '{$reporte}', NOW(), 'Nuevo', NOW(), 0, 'Sin Asignar', '{$li}', '{$ipO}', '{$telefono}', '{$nUsuario}', '{$ncontrasena}', '{$nIpMaquina}', '{$depto}');";
			}  else {
				$nombre = ($nombreOverride !== '') ? $nombreOverride : $matr;
				$li     = '<li class="smaller">'.date("Y-m-d H:i:s").' | Reporte enviado por: '.$nombre.'</li>';
				$query  = "INSERT INTO cdi_reportes_manual (NRastreo, nombreReportador, eMail, nSerie, descripcionFalla, fechaReporte, status, ultimoCambio, tiempoTranscurrido, soporte, historial, ipOrigen, Telefono, usuario, contrasena, ipEquipo, departamento) VALUES ";
				$query .= "('{$id}', '{$nombre}', '{$correo}', '{$serie}', '{$reporte}', NOW(), 'Nuevo', NOW(), 0, 'Sin Asignar', '{$li}', '{$ipO}', '{$telefono}', '{$nUsuario}', '{$ncontrasena}', '{$nIpMaquina}', '{$depto}');";
			}
			$res = Yii::app()->db->createCommand($query)->execute();
			return ($res != 0) ? true : false;
		}
